...
export default function Dashboard() {
  ...
  if (loading) return (
    <div className="flex items-center justify-center min-h-[60vh]">
      <div className="text-center">
        ...
        <p className="text-gray-400 text-sm">کھاتہ لوڈ ہو رہا ہے...</p>
        <p className="text-gray-300 text-xs mt-1">Loading your khata</p>
      </div>
    </div>
  );
  ...
  const avgScore = customers.length
    ? Math.round(customers.reduce((s, c) => s + c.aitbaar_score, 0) / customers.length)
    : 0;

  const chartData = cf?.upcoming_collections?.map((c) => ({
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="bg-green-950 rounded-2xl p-6 md:p-8 flex items-end justify-between min-h-[160px] relative overflow-hidden">
          ...
          <div>
            <p className="text-white/50 text-xs uppercase tracking-widest mb-1">کل باقی رقم</p>
            <p className="text-white/30 text-xs mb-2">Total Outstanding</p>
            <p className="text-4xl md:text-5xl font-bold text-white"
              style={{ fontFamily: "Cormorant Garamond, serif" }}>
              {formatAmount(totalDue)}
            </p>
            <p className="text-white/40 text-sm mt-2">
              {customers.length} گاہکوں میں — across {customers.length} customers
            </p>
          </div>
          <div className="text-right">
            <div className="bg-white/10 border border-white/20 rounded-xl px-3 py-1.5">
              <p className="text-white/60 text-xs">اوسط اعتبار</p>
        ...
        <div className="bg-white border border-stone-200 rounded-2xl p-6 md:p-8 flex items-end justify-between min-h-[160px] relative overflow-hidden">
          ...
          <div>
            <p className="text-gray-400 text-xs uppercase tracking-widest mb-1">خطرے میں رقم</p>
            <p className="text-gray-300 text-xs mb-2">At Risk Amount</p>
            <p className="text-4xl md:text-5xl font-bold text-red-600"
              style={{ fontFamily: "Cormorant Garamond, serif" }}>
              {formatAmount(cf?.at_risk_amount || 0)}
            </p>
            <p className="text-gray-400 text-sm mt-2">
              {atRiskCount} خطرناک · {safeCount} قابل اعتبار
            </p>
          </div>
          {cf?.shortage_warning && (
            <div className="bg-red-50 border border-red-200 rounded-xl px-3 py-1.5 text-right">
              <p className="text-red-500 text-xs font-semibold">⚠ خبردار</p>
              <p className="text-red-400 text-xs">نقد کمی</p>
            </div>
          )}
        </div>
      </div>
      ...
      {cf?.shortage_warning && (
        <div className="bg-orange-50 border border-orange-200 rounded-2xl p-5 flex gap-4 items-start">
          <img src={emergency} alt="" className="w-10 h-10 mt-0.5 flex-shrink-0" />
          <div>
            <p className="font-semibold text-orange-900 mb-1">
              ⚠ نقد رقم کی کمی کا خطرہ — Cash Flow Warning
            </p>
            <p className="text-orange-700 text-sm leading-relaxed">
              آپ کی کل باقی رقم کا 30% سے زیادہ حصہ خطرناک گاہکوں کے پاس ہے۔
              اس ہفتے نئی ادھار دینے سے پہلے پرانی وصولی کریں۔
            </p>
            <p className="text-orange-500 text-xs mt-1">
              Over 30% of outstanding is with high-risk customers. Prioritize collections first.
            </p>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
          ...
          <div className="flex items-start justify-between mb-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-gray-400 text-xs">Upcoming Collections</p>
              </div>
            </div>
            {chartData.length > 0 && (
              <div className="text-right">
                <p className="text-xs text-gray-400">کل متوقع</p>
          ...
          {chartData.length > 0 ? (
            <ResponsiveContainer width="100%" height={200}>
              <AreaChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                ...
                <XAxis dataKey="name" tick={{ fontSize: 11, fill: "#9ca3af" }} axisLine={false} tickLine={false} dy={8} />
                <YAxis tick={{ fontSize: 10, fill: "#9ca3af" }} axisLine={false} tickLine={false}
                  tickFormatter={(v) => `₨${v >= 1000 ? `${(v / 1000).toFixed(0)}k` : v}`} />
                <Tooltip
                  contentStyle={{ borderRadius: "12px", border: "1px solid #e7e5e4", boxShadow: "0 4px 20px rgba(0,0,0,0.08)", fontSize: "0.8rem" }}
                  formatter={(v) => [`₨${v.toLocaleString()}`, "باقی رقم"]}
                  labelStyle={{ fontWeight: 600, color: "#1c1917" }}
                />
                <Area type="monotone" dataKey="amount" stroke="#052e16" strokeWidth={2.5}
                  fill="url(#greenGrad)"
            ...
            <div className="flex flex-col items-center justify-center h-48 text-gray-300">
              <img src={cashflow} alt="" className="w-10 h-10 mb-2 opacity-20" />
              <p className="text-sm text-gray-400">ابھی کوئی وصولی نہیں</p>
              <p className="text-xs text-gray-300 mt-0.5">No upcoming collections</p>
            </div>
          )}
        </div>
        ...
        <div className="md:col-span-2 bg-green-950 rounded-2xl p-6 flex flex-col justify-between">
          <div>
            ...
            <p className="text-white/40 text-xs mb-4">Financial Advisor</p>
            <p className="text-white/70 text-sm leading-relaxed italic">
              {cf?.ai_insight || "GitHub token .env میں شامل کریں تاکہ AI مشورے فعال ہوں۔"}
            </p>
          </div>
          <button
            onClick={() => navigate("/ai")}
          ...
          <div className="p-5 border-b border-stone-100 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-gray-400 text-xs">Customers Needing Attention</p>
              </div>
            </div>
            <span className="bg-red-50 text-red-600 text-xs font-bold px-2.5 py-1 rounded-full">
              {cf.customers_at_risk.length} نشاندہی شدہ
            </span>
          </div>
              ...
              >
                <div className="flex items-center gap-3 min-w-0">
                  <div className={`w-9 h-9 rounded-xl flex items-center justify-center text-sm font-bold flex-shrink-0 ${
                    c.aitbaar_score < 40 ? "bg-red-100 text-red-700" : "bg-orange-100 text-orange-700"
                  }`}>
                    {c.customer_name[0]}
                  </div>
                  <div className="min-w-0">
                    <p className="font-semibold text-gray-900 text-sm truncate">{c.customer_name}</p>
                    <p className="text-gray-400 text-xs mt-0.5">{c.risk_reason}</p>
                  </div>
                </div>
                ...
                <div className="hidden md:flex items-center gap-2 mx-4">
                  ...
                  <span className={`text-xs px-2.5 py-1 rounded-full font-bold ${
                    ...
                    اعتبار {c.aitbaar_score}
                  </span>
                </div>

                <div className="flex items-center gap-3 flex-shrink-0">
                  <p className="text-red-600 font-bold text-sm">{formatAmount(c.amount_due)}</p>
                  <span className="text-gray-300 group-hover:text-green-900 transition-colors text-sm">→</span>
                </div>
              </div>
            ))}
          </div>
